...
public:
	...
	explicit
	binary_indexed_tree(const combination_type& comb = combination_type()) : comb(comb) {}

	binary_indexed_tree(const binary_indexed_tree& copied) : comb(copied.comb) {
	...
	binary_indexed_tree(const value_type& repeated_value,
		const combination_type& comb = combination_type()) : comb(comb) {
	...
	binary_indexed_tree(InputIterator first, const combination_type& comb = combination_type());
